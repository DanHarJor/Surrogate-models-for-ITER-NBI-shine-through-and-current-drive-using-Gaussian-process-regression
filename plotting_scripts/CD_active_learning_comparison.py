...
def plot_sampling_strategy_comparison(
    ...
    if num_samples is None:
        raise ValueError('You must specify num_samples, it must be the same within each strategy for the aggregation to work')
    ...
    for strategy in combined["strategy"].unique():
        ...
        if 'random' in strategy:
            n = 10000
            num_samples = np.array([50 * (i + 1) for i in range(n)]) / 10
        else:
            n = 10000
            num_samples = np.array([50, 60, 70] + [70 + 50 * (i + 1) for i in range(n)]) / 10
    ...
    for base_metric, variants in metric_groups.items():
        ...
        if plot_mode == "individual":
            for strategy in combined["strategy"].unique():
                ...
                if base_metric == 'rmse':
                    y = y / 1e6
                    ax.set_ylabel('RMSE, CD [MA]')
                    ax.hlines([10560 / 1e6], min(x), max(x))
                    if y[-1] < best_rmse:
                        best_rmse = y[-1]
                        print(f'Best RMSE: {strategy}')
                ax.plot(
                    x, y,
                    marker='o',
        ...
        # ----------------------------------------------------
            ...
            for strategy in unique_norm:
                ...
                # Shaded region: from the minimum up to mean + std
                ax.fill_between(
                    x,
                    y_min,
                    y_mean + y_std,
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(output_dir, f"CD_{base_metric}_{plot_mode}.png"), dpi=300)
        plt.close(fig)
...
def main():
    ...
    data_folder = os.path.join(script_dir, "..", "current_drive_data_sets", "active_learning_comparison")
    ...
    n = 100000
    num_samples = np.array([50 * i for i in range(1, n + 1)])
