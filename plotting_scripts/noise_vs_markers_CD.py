...
# Load the CSV
input_csv = os.path.join(script_dir, "..", "current_drive_data_sets", "noise_vs_nmarkers", "enchanted_dataset.csv")
do_violin = True
do_annotate = False

remove_marker_quantity = [10, 100]

true_values = [] # {1000: 0.255490, 10000: 0.253318, 100000: 0.252822}

df = pd.read_csv(input_csv)

df = df[~df["marker_quantity"].isin(remove_marker_quantity)]


# Validate required columns
required = {"current_drive_A", "marker_quantity", "ascot_runtime_min"}
missing = required - set(df.columns)
if missing:
    raise ValueError(f"Missing required columns: {missing}")

out_col = "current_drive_A"

# Extract raw data
x = df["marker_quantity"]
y = df[out_col]

# Compute total CPU hours per marker count
cpumin = df.groupby("marker_quantity")["ascot_runtime_min"].mean()
...
mean_shine = df.groupby("marker_quantity")[out_col].mean() * scale_factor
# Compute SEM for each marker count
sem_shine = df.groupby("marker_quantity")[out_col].sem() * scale_factor

std_shine = df.groupby("marker_quantity")[out_col].std() * scale_factor
...
ax1.set_ylabel("Current Drive MA")
...
marker_vals = np.sort(df["marker_quantity"].unique())

if do_violin == True:
    for m in marker_vals:
        subset = df.loc[df["marker_quantity"] == m, out_col].values * scale_factor
...
for m in marker_vals:
    ...
    # Draw 10 random samples for this marker count
    sample_means = []
    for _ in range(10):
    ...
    bootstrap_sems.append(np.std(sample_means) / np.sqrt(10))  # SEM of the bootstrap means
...
print(f'95% CI {[b * 1.96 for b in bootstrap_sems]}')
...
ax3.set_ylabel("Current Drive MA")
...
ax2.set_ylabel("ASCOT, 128 cores [min]", color=okabe_ito[1])
...
output_png = os.path.join(script_dir, "plots", "noise_vs_markers_CD.png")
